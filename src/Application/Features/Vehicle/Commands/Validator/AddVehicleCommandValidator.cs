...
namespace Auction.Application.Features.Vehicle.Commands.Validator;
...
public class AddVehicleCommandValidator : AbstractValidator<AddVehicleCommand>
{
    public AddVehicleCommandValidator()
    {
        ...
        When(x => x.Type == nameof(VehicleType.SUV), () =>
        {
            RuleFor(x => x.NumberOfSeats).NotNull().InclusiveBetween(2, 10);
        });
        When(x => x.Type == nameof(VehicleType.Truck), () =>
        {
